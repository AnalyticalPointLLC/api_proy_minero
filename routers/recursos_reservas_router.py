...
@router.get("/completos/{type_column}/{year_data}", response_model=List[RecursoReservaCompleto])
def read_recursos_reservas_completos(type_column: str, year_data: int, skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    try:
        recursos_reservas_completos = get_recursos_reservas_completos(db, type_column, year_data, skip=skip, limit=limit)
